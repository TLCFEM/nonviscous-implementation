...
\bw=\mb{T}\bv,
\end{gather}
where $\mb{T}$ picks the participating DoFs, and can be, for example, a square diagonal matrix with its diagonal entries being either one or zero. \eqsref{eq:wv} is beneficial when it comes to compositing flexible damping based on different node/element-based rules. It will be discussed later in this work.
...
\begin{Objective}
%\begin{rmk}
Since the implicit trapezoidal rule is A-stable, the overall result is stable as long as the adopted Newmark method is stable, namely, $\gamma\geqslant0.5$ and $\beta\geqslant0.25\left(\gamma+0.5\right)^2$. Accounting for the fact that the Newmark method can be second-order accurate ($\gamma=0.5$ and $\beta=0.25$), the implicit trapezoidal rule is adopted here to improve the overall accuracy.

If other direct time integration methods are used for \eqsref{eq:eom}, other methods can be applied to discretise \eqsref{eq:single_conv_ode}. The following are some examples.
\begin{enumerate}
