...
Assuming \eqsref{eq:eom} is satisfied at $t_{n+1}$, then, accounting for \eqsref{eq:discretised_c}, \eqsref{eq:eom} is
...
By using partial sums and model reduction, \citet{Gao2022} presented such a method, named as VPMR, with controllable magnitudes of exponents, formally,
\begin{gather}
...
By using VPMR, it is feasible to convert nonviscous damping with arbitrary (in terms of both number and form) kernels applied to the dynamic system into the form of \eqsref{eq:composition}.
