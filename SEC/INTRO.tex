...
\begin{enumerate}
\item For exponential kernels or kernels can be expressed as sums of exponentials, a nonviscously damped system can be equivalently rewritten into a second-order ODE and a first-order ODE. Both can be discretised and solved directly in the time domain with a performant formulation.
...
\end{enumerate}
Numerical examples are presented with an emphasis on accuracy by comparing with analytical solutions (if available).
