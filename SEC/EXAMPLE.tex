...
\end{gather}
\add{It shall be pointed out that, due to conventions, $m$ is used to represent the mass of the oscillator in the current context. The same symbol has been used in the previous derivation, see, e.g., \eqsref{eq:single_conv}. Please note the different meanings in various contexts.}
The parameters are set to $m=1$, $c=2$, $\mu=1$, $k=100$.
The initial conditions are $u\left(0\right)=1$, $\dot{u}\left(0\right)=0$ and $\ddot{u}\left(0\right)=-100$\replace{.}{for free vibration, namely, $p\left(t\right)=0$.} The \add{corresponding} closed form solution can be \replace{found}{derived, the procedure is presented} in \secref{sec:analytical_sdof}.
...
\add{Following a similar presentation elsewhere \citep{Liu2023}, the absolute error history is shown in \figref{fig:sdof_error}.
\begin{figure}[H]
\centering
...
\add{Regarding the computational efficiency, \tabref{tab:sdof_cost} presents the wall clock time required to perform an analysis for \SI{10}{\second} with different time step sizes.
...
\add{The absolute error history is shown in \figref{fig:sdof_forced_error}.
\begin{figure}[H]
\centering
...
\end{gather}
can be used. The sum-of-exponential approximations given by the VPMR algorithm are shown in \figref{fig:vpmr}. With a \add{chosen} tolerance around \num{e-12}, only \num{13} exponentials are required. The corresponding $m_l$ and $s_l$ parameters are given in \secref{sec:vpmr}.
\begin{figure}[H]
\centering
...
\add{In terms of the computational efficiency, \tabref{tab:three_cost} lists the wall clock time required to perform an analysis for \SI{50}{\second} with different time step sizes.
